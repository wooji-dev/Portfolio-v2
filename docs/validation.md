...
## 2026-09-13 글라스 오브제 교체

- Pinterest 레퍼런스를 직접 열어 형태와 재질을 확인하고 Higgsfield에서 두 이미지를 새로 생성. 원본 1024 × 1024와 웹용 JPEG의 실제 로딩 확인.
- 1280px, 800px, 390px, 320px에서 오브제 배치와 배경 연결을 시각 확인. 가로 넘침 없음.
- 1280px 및 390px에서 실제 드래그 후 위치 변경과 포인터 해제 확인. 두 오브제 간 밀어내기 및 관성 반응 유지.
- Enter 조작 후 회전 변경, 다시 모으기, Motion off의 위치·각도 초기화와 버튼 비활성화, Motion on의 재활성화 확인.
- 여섯 프로젝트 표지의 데스크톱 크기 545 × 368px 유지. 기존 Figma MCP, A/B 테스트, solvPS 표지 및 소제목 규칙은 변경하지 않음.
- `npm run build` 및 `git diff --check` 통과. 브라우저 경고 및 오류 없음. 실제 터치 기기 검증은 수행하지 않음.
