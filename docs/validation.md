...
## 2026-09-13 사용자 제공 solvPS 스크린샷으로 교체

- 사용자 제공 2880 × 1800 PNG를 같은 해상도의 JPEG로 인코딩. 원본 화면과 비율 유지.
- 상세 영역의 699px 제한을 제거. 데스크톱에서 약 950 × 595px, 390px 모바일에서 342 × 215px로 표시되며 가로 넘침 없음.
- 새 이미지 경로와 대체 텍스트 확인. 브라우저 경고 및 오류 없음.
- `npm run build`와 `git diff --check` 통과.

## 2026-09-13 solvPS 상세 이미지 추가

- 기존 포트폴리오 PDF의 solvPS 섹션과 포함 이미지를 시각 확인하고 노트북 목업을 추출해 재사용.
- 1280px에서 원본 699 × 432px로 로딩, 390px에서 342 × 211px로 비율을 유지하며 축소됨. 상세 창의 가로 넘침 없음.
- 캡션과 대체 텍스트 표시, `#solvps` 직접 진입, Esc 닫기 및 `#work` 복귀 확인.
- `npm run build`와 `git diff --check` 통과. 브라우저 경고 및 오류 없음.
